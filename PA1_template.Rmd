...
In this document, we will present you the 2 months of activity's steps for an anonymous individual. Each measurement are made with an interval of 5 minutes which result of **17 568 observations**.
...
## Analyses of the step's number each day
...
In the second time, the median and mean:
...
At last, We will observer the 5 minutes interval that, on average, contains the maximum of steps:
...
```
We found for answer: `r meanMaxInterval`

# Handle missing values

To well understand the importance of missing values imputing strategie we will first measured the number of missing value present in the current dataset.
...
There is a lot of strategies to handle missing values. In the code above, we choose the strategies of simply discarding the value.
...
For the rest of this document I choose to use the **Simple missing-data approaches that retain all the data**. This method does not change the size of the dataset. The missing values are simply replaced by a new one. Differents startegies are used to choose the value. In this example, we will just replace it by **the mean for that 5 min interval**.
...
With that strategy, we will be able to observer the impact of the imputation method on the final computation.
...
And we compute the mean and the median of those steps by days:
...
We can see that the results are really differents. Particullary on the computation of the median and the mean.
